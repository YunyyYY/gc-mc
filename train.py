...
# add cold start args
ap.add_argument('-nc', '--num_cold_start', type=int, default=0,
                help="Number of cold start nodes")
ap.add_argument('-nr', '--num_rating', type=int, default=-1,
                help="Number of ratings preserved, -1 means all")
...
args = vars(ap.parse_args())  # default of parse_args() input is taken from sys.argv.
...
if DATASET == 'ml_1m' or DATASET == 'ml_10m':
    if FEATURES:
        datasplit_path = 'data/' + DATASET + '/withfeatures_split_seed' + str(DATASEED) + ncnr + '.pickle'
    else:
        datasplit_path = 'data/' + DATASET + '/split_seed' + str(DATASEED) + ncnr + '.pickle'
elif FEATURES:
...
if DATASET == 'flixster' or DATASET == 'douban' or DATASET == 'yahoo_music':
    ...
    u_features, v_features, adj_train, train_labels, train_u_indices, train_v_indices, \
        val_labels, val_u_indices, val_v_indices, test_labels, \
        test_u_indices, test_v_indices, class_values = load_official_trainvaltest_split(DATASET, NC, NR, TESTING)
else:
