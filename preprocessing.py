...
def load_official_trainvaltest_split(dataset, nc, nr, testing=False):
    """
    ...
    data_array_train = data_train.iloc[:,:].values.tolist()
    data_array_train = np.array(data_array_train)
    data_array_test = data_test.iloc[:,:].values.tolist()
    data_array_test = np.array(data_array_test)

    # randomly select Nc users, each only keep at most Nr ratings
    import random
    _u_nodes_ratings = data_array_train[:, 0].astype(dtypes['u_nodes'])
    _v_nodes_ratings = data_array_train[:, 1].astype(dtypes['v_nodes'])
    _ratings = data_array_train[:, 2].astype(dtypes['ratings'])
    _dummy = data_array_train[:, 3]  # useless
    unique_users = list(np.unique(_u_nodes_ratings))
    cold_users = random.sample(unique_users, nc)
    count_nr = np.zeros(nc)
    u_nodes_ratings = []
    v_nodes_ratings = []
    ratings = []
    dummy = []

    for i, u in enumerate(_u_nodes_ratings):
        if u in cold_users:
            iu = cold_users.index(u)
            if count_nr[iu] >= nr:
                continue
            count_nr[iu] += 1
        u_nodes_ratings.append(u)
        v_nodes_ratings.append(_v_nodes_ratings[i])
        ratings.append(_ratings[i])
        dummy.append(_dummy[i])
    data_array_train = np.vstack([u_nodes_ratings, v_nodes_ratings, ratings, dummy]).transpose()
    ...
    labels = np.full((num_users, num_items), neutral_rating, dtype=np.int32)
    # initialize labels of shape (num_users, num_items) and all terms equal to <neutral_rating>
    labels[u_nodes, v_nodes] = np.array([rating_dict[r] for r in ratings])
    ...
    if testing:
        u_train_idx = np.hstack([u_train_idx, u_val_idx])   # stack horizontally for arrays of the same dimension
        v_train_idx = np.hstack([v_train_idx, v_val_idx])   # and having same number of items along axis=0
        train_labels = np.hstack([train_labels, val_labels])
        # for adjacency matrix construction
        train_idx = np.hstack([train_idx, val_idx])
